...
public class MyPanel extends JPanel implements Runnable {
    ...
    public void paint(Graphics g) {
        super.paint(g);	//不清空原有绘制，在原来图像基础上继续绘制
        ...
        for (Person person : people) {
            ...
            g.fillOval(person.getX(), person.getY(), 3, 3); //绘制代表人的圆点
        ...
        g.setColor(new Color(0xff0000));
        g.drawString("city1发病者人数：" + PersonPool.getInstance().getPeopleSizeByCity(Person.State.CONFIRMED, Constants.CITY1), captionStartOffsetX, captionStartOffsetY + 3 * captionSize);
        g.setColor(new Color(0xff0000));
        g.drawString("city2发病者人数：" + PersonPool.getInstance().getPeopleSizeByCity(Person.State.CONFIRMED, Constants.CITY2), captionStartOffsetX, captionStartOffsetY + 4 * captionSize);
        g.setColor(new Color(0x48FFFC));
        ...
        //暂定缺少病床数量为 NEED = 总床数 - 确诊发病者数量 - 已隔离住院数量
        //
        int needBeds = Constants.BED_COUNT - PersonPool.getInstance().getPeopleSize(Person.State.CONFIRMED)
        		- PersonPool.getInstance().getPeopleSize(Person.State.FREEZE);
     

        g.drawString("缺少病床：" + (needBeds > 0 ? 0: -needBeds), captionStartOffsetX, captionStartOffsetY + 7 * captionSize);
    ...
    @Override
    public void run() {
    	//设置时间比例，每一秒一天，每0.1秒统计一次 (worldTime / 10.0)
